...
1. url到html的过程
    ...
    -  渲染页面
        -  html生成DOM tree
        -  CSS生成CSSOM树
        -  DOM tree和CSSOM 树整合渲染树RenderTree
        -  根据RenderTree开始渲染和展示
        -  遇到script标签会执行并阻塞渲染
3. window.onload和DOMContentLoaded
   ```javascript
   window.addEventListener('load',function(){
     //页面的全部资源加载完才会执行，包括图片和视频等
   })
   document.addEventListener('DOMContentLoaded',function(){
      //DOM渲染完成即可执行，图片和视频等可能还没有加载完
   })
   ```
# 性能优化
1.  原则
...
3.  入手点
    -  加载页面和静态资源
       -  静态资源压缩合并
       -  静态资源缓存(通过链接名称控制缓存，内容改变，链接的名称才会改变)
       -  使用CDN加速资源
       -  SSR后端渲染，数据直接输出到HTML中。（JSP，PHP，ASP都属于后端渲染）
    -  页面渲染
       -  CSS在head标签中，JS在body标签结束之前
       -  懒加载（图片懒加载，下载加载更多）
       -  减少DOM查询，缓存DOM查询结果
       -  减少DOM操作，多个操作合并执行（创建一个DOM片段，document.createDocumentFragment()）
       -  事件节流
       -  尽早执行操作，如DOMContentLoaded
# 安全
1.  XSS跨站请求攻击
    -  攻击代码中含有获取cookie的作用，发送给攻击者服务器，得到用户信息
    -  解决：前端替换关键字，例如<为&lt;>为&gt;
2.  XSRF跨站请求伪造
    - 钓鱼邮件，偷偷付费
    - 解决：增加验证过程，如输入密码，指纹，短信验证码
